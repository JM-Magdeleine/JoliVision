...
class CPMVQwenVLM(nn.Module):
    ...
    def train_projector(
        ...
        if dataset is None:
            ...
            output = self.forward(image=image, text=description, target=description)
            print(output)
...
# vlm.train_projector()
print(inspect.getsource(tokenizer.apply_chat_template))
...
class CPMVQwenVLM(nn.Module):
    ...
    # Fix, give the embeddings thru lm_model.generate
    def generate(self, text: str, image: str):
        mm_embeds = self.embed_mixed_modal(text, image)
